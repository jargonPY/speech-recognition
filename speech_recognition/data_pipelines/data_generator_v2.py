...
class DataGeneratorV2(tf.keras.utils.Sequence):
  ...
  def __init__(self, mode="train", split=0.8, batch_size=64, one_hot=True):
  ...
  def __getitem__(self, index):
    ...
    logger.info(f"Shapes: {encoder_input.shape, decoder_input.shape, decoder_output.shape}")
  ...
  def get_file_names(mode, split):
    ...
    def fetch_names(audio_path, text_path):
      audio_files = [file.path for file in os.scandir(os.path.abspath(audio_path))]
      text_files = [file.path for file in os.scandir(os.path.abspath(text_path))]
    ...
    if mode == "train":
      audio_files, text_files = fetch_names(config.AUTIO_TRAIN_PATH, config.TEXT_TRAIN_PATH)
